...
#    (containing the code I provided in the previous responses) are in your current working directory in the notebook environment.
#    You may need to upload them manually to your Colab/Vast.ai file browser or use `git clone` and then replace the files in the cloned directory.
# 3. Run the cells sequentially by clicking the play button next to each cell or using Shift+Enter.
# 4. **Modify the `MODELS` dictionary** in the second code cell (`app.py` code) to include the Hugging Face IDs of your desired NSFW SDXL models. **Remember to replace the placeholder IDs.**
...
# @markdown This cell contains the complete `app.py` code with modifications for custom model loading and launches the Gradio interface.
# @markdown **Ensure `controlnet_union.py` and `pipeline_fill_sd_xl.py` are in your current directory.**
# @markdown **Modify the `MODELS` dictionary below to include the Hugging Face IDs of your desired NSFW SDXL model IDs.**

# --- Start of app.py code ---
import spaces
import gradio as gr
import torch
...
# Make sure controlnet_union.py is in the same directory
try:
    from controlnet_union import ControlNetModel_Union
except ImportError:
    print("Error: controlnet_union.py not found. Please ensure it's in the same directory.")
    # You might need to exit here or handle the error appropriately
    ControlNetModel_Union = None # Set to None to prevent further errors


# Make sure pipeline_fill_sd_xl.py is in the same directory
try:
    from pipeline_fill_sd_xl import StableDiffusionXLFillPipeline
except ImportError:
    print("Error: pipeline_fill_sd_xl.py not found. Please ensure it's in the same directory.")
    # You might need to exit here or handle the error appropriately
    StableDiffusionXLFillPipeline = None # Set to None
...
# Define the models you want to use, including your NSFW ones
# Replace the placeholder IDs with the actual Hugging Face model IDs (or local paths)
MODELS = {
    ...
    # Add your desired NSFW SDXL models here (replace with actual IDs from Hugging Face or local paths)
    "MyFavoriteNSFWModel": "user/my-favorite-nsfw-sdxl-model-id", # <-- REPLACE WITH ACTUAL MODEL ID
    "AnotherHotModel": "another_user/another-spicy-model", # <-- REPLACE WITH ACTUAL MODEL ID
    # ... add more as needed
}

# Load the fixed VAE (can be loaded once)
# Use try-except to handle potential errors during loading
vae = None # Initialize VAE to None
try:
    print("Loading VAE...")
    ...
    vae = None # Set VAE to None if loading fails


# Load the custom ControlNet (can be loaded once)
controlnet_instance = None # Initialize ControlNet to None
# Only attempt to load ControlNet if ControlNetModel_Union was imported successfully
if ControlNetModel_Union is not None:
    try:
        print("Loading ControlNet...")
        # Ensure the directory for ControlNet files exists - hf_hub_download handles caching
        ...
        # Note: The _load_pretrained_model is a bit of an internal method. A more standard approach
        # would be ControlNetModel_Union.from_pretrained(controlnet_repo_id, filename="diffusion_pytorch_model_promax.safetensors")
        # if the safetensors file contains the full model state_dict in the expected format.
        # Sticking to the original code's method for compatibility for now.
        ...
        controlnet_instance.to(device="cuda", dtype=torch.float16)
        print("ControlNet loaded successfully.")
    except Exception as e:
        print(f"Error loading ControlNet: {e}")
        controlnet_instance = None # Set ControlNet to None if loading fails
...
# Function to load pipeline components for a given model ID
def load_pipeline_for_model(model_id):
    global pipe # Declare pipe as global to modify the outer scope variable
    print(f"Attempting to load pipeline components for model: {model_id}")

    if StableDiffusionXLFillPipeline is None or vae is None or controlnet_instance is None:
        print("Error: Required modules (Pipeline, VAE, or ControlNet) not loaded. Cannot load pipeline.")
        return None

    # Check if the requested model is already loaded
    if pipe is not None and hasattr(pipe, 'pretrained_model_name_or_path') and pipe.pretrained_model_name_or_path == model_id:
        print(f"Pipeline for {model_id} is already loaded.")
        return pipe

    # If a different model is loaded, clean up before loading the new one
    if pipe is not None:
        ...
        try:
            ...
            print("Existing pipeline offloaded.")
        except Exception as e:
             print(f"Error during pipeline offloading: {e}")


    try:
        # Load components from the selected model ID
        print(f"Loading UNet, Text Encoders, and Tokenizers for {model_id}...")
        ...
        text_encoder_2 = CLIPTextModelWithProjection.from_pretrained(model_id, subfolder="text_encoder_2", torch_dtype=torch.float16).to("cuda")
        tokenizer = CLIPTokenizer.from_pretrained(model_id, subfolder="tokenizer")
        tokenizer_2 = CLIPTokenizer.from_pretrained(model_id, subfolder="tokenizer_2")
        print("Components loaded.")

        # Create the custom pipeline with the loaded components
        ...
        pipeline = StableDiffusionXLFillPipeline(
            ...
            unet=unet,
            controlnet=controlnet_instance, # Use the custom ControlNet
            scheduler=TCDScheduler.from_config(unet.config.scheduler_config) # Use scheduler config from the UNet
        )
        # Ensure scheduler is TCDScheduler as used in the original code
        pipeline.scheduler = TCDScheduler.from_config(pipeline.scheduler.config)
        pipeline.to("cuda")
        # Store the model ID in the pipeline object for easy checking
        pipeline.pretrained_model_name_or_path = model_id
        print(f"Pipeline loaded successfully for {model_id}")
        return pipeline
    except Exception as e:
        print(f"Error loading pipeline for {model_id}: {e}")
        return None # Return None if loading fails

# Initialize the pipeline with the default model on startup if required components are loaded
if StableDiffusionXLFillPipeline is not None and vae is not None and controlnet_instance is not None:
    pipe = load_pipeline_for_model(list(MODELS.values())[0]) # Load the first model in the dictionary by default
else:
    print("Skipping initial pipeline load because required components failed to load.")
