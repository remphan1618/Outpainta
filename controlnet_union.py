...
class ControlNetModel_Union(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    Args:
        ...
        num_attention_heads (`Optional[Union[int, Tuple[int]]]`, defaults to 8):
        ...
        num_control_type (`int`, defaults to 6):
    ...
    def forward(
        ...
        if self.config.addition_embed_type is not None:
            if self.config.addition_embed_type == "text":
                if encoder_hidden_states is not None:
                    ...
                    logger.warn("encoder_hidden_states is None but addition_embed_type is 'text'. Skipping addition embedding.")
        ...
        # class embedding (moved below addition embedding based on typical SDXL structure)
        ...
        if control_type is None:
             logger.warn("`control_type` is None in `added_cond_kwargs`. Control type injection will be skipped.")
             control_emb = None
        else:
        ...
        # Check if control_type is available and has expected shape before processing
        if control_type is None or controlnet_cond_list is None or control_type.shape[0] == 0:
            logger.warn("`control_type` or `controlnet_cond_list` is missing/empty. Skipping Condition Transformer.")
            # If control data is missing, just use the processed sample as the initial down_block_res_samples
            ...
            for idx in range(indices.shape[0] + 1):
                if idx == indices.shape[0]:
                    # This path processes the initial noisy sample feature
                    controlnet_cond = sample
                    feat_seq = torch.mean(controlnet_cond, dim=(2, 3))  # N * C
                else:
                    # This path processes the specific control condition image
                    # Ensure index is within bounds of controlnet_cond_list
                    if indices[idx][0] < len(controlnet_cond_list):
                         controlnet_cond = self.controlnet_cond_embedding(
                             controlnet_cond_list[indices[idx][0]]
                         )
                         feat_seq = torch.mean(controlnet_cond, dim=(2, 3))  # N * C
                         # Ensure index is within bounds of task_embedding
                         if indices[idx][0] < self.task_embedding.shape[0]:
                              feat_seq = feat_seq + self.task_embedding[indices[idx][0]]
                         else:
                              logger.warn(f"Index {indices[idx][0]} out of bounds for task_embedding.")
                              # Handle out of bounds by not adding task embedding or logging an error

                    else:
                         logger.warn(f"Index {indices[idx][0]} out of bounds for controlnet_cond_list. Skipping condition.")
            ...
            # Only proceed with transformer and fusion if there are valid inputs
            if inputs:
                x = torch.cat(inputs, dim=1)  # NxLxC
                x = self.transformer_layes(x)

                controlnet_cond_fuser = sample * 0.0
                # Iterate only up to the number of valid condition inputs
                for idx in range(len(condition_list) -1 ): # Exclude the sample itself which is the last in condition_list

                     # Ensure index is within bounds of spatial_ch_projs output
                     if idx < x.shape[1]:
                         alpha = self.spatial_ch_projs(x[:, idx])
                         alpha = alpha.unsqueeze(-1).unsqueeze(-1)
                         controlnet_cond_fuser += condition_list[idx] + alpha
                     else:
                         logger.warn(f"Index {idx} out of bounds for transformer output. Skipping fusion for this index.")


                sample = sample + controlnet_cond_fuser
                down_block_res_samples = (sample,) # Initialize with the fused sample

            else:
                logger.warn("No valid inputs generated for Condition Transformer. Skipping transformer and fusion.")
                down_block_res_samples = (sample,) # Initialize with the original processed sample


        # 3. down
        # Ensure down_block_res_samples has been initialized
        if not down_block_res_samples:
             down_block_res_samples = (sample,) # Default if somehow not initialized above
        ...
        # Ensure the number of controlnet_down_blocks matches down_block_res_samples before zipping
        if len(self.controlnet_down_blocks) != len(down_block_res_samples):
             logger.error(f"Mismatch in number of controlnet_down_blocks ({len(self.controlnet_down_blocks)}) and down_block_res_samples ({len(down_block_res_samples)}). This might cause errors.")
             # Attempt to proceed with the minimum number of blocks to avoid immediate crash
             num_blocks_to_process = min(len(self.controlnet_down_blocks), len(down_block_res_samples))
             blocks_to_process = zip(down_block_res_samples[:num_blocks_to_process], self.controlnet_down_blocks[:num_blocks_to_process])
        else:
             blocks_to_process = zip(down_block_res_samples, self.controlnet_down_blocks)


        for down_block_res_sample, controlnet_block in blocks_to_process:
